 ...
 (function() {
     ...
     function generateHashOffset(section){
        ...
        Array.prototype.forEach.call(section, function(e) {
         ...
         console.log(sections[hash],hash);
     });
        return sections;
     }
     var stickyObject = document.getElementById("fixed-toc"),
         content = document.getElementById("post-content"),
         start = stickyObject.offsetTop,
         //start = stickyObject.offsetTop + stickyObject.offsetHeight,
         stop =  outerHeight(content)-outerHeight(stickyObject),
    ...
    window.onresize = function(event) {
     ...
     window.onscroll = function(e) {
         ...
         if (scrollPosition >= start && scrollPosition <= stop) {
             // stick the div        
             stickyObject.className = 'is-sticky';
             stickyObject.removeAttribute("style");
         } else {
             if (scrollPosition >= stop) {
				 //console.log('stickypos: '+ scrollPosition);
                 stickyObject.className = 'is-sticky-end';
                 stickyObject.style["top"] = stop - scrollPosition + 'px';
             } else {
                 (document.querySelector('#my_toc .active') != null) ? document.querySelector('#my_toc .active').setAttribute('class', ' '): null;
                 // release the div
                 stickyObject.className = '';
                 stickyObject.removeAttribute("style");
             }
         }
         for (i in sections) {
             if (sections[i] <= scrollPosition) {
				 console.log(sections[i],scrollPosition);
                 (document.querySelector('#my_toc .active') != null) ? document.querySelector('#my_toc .active').setAttribute('class', ' '): null;
